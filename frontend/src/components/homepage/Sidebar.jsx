...
const Sidebar = () => {
  ...
  const getTopCommunities = async () => {
    const topCommunities = await axios({
      method: "get",
      url: "http://localhost:5000/community/top",
      withCredentials: true,
    });
    return topCommunities.data;
  };

  const { data: t_communities, isLoading } = useQuery(
    "topCommunities",
    getTopCommunities
  );
    ...
    <div id={increaseZIndex ? "increaseIndex" : ""} className="sidebar">
      <div className="sticky-container">
        <button
          ...
          className="create-space-btn-two"
        >
