...
const R_Sidebar = () => {
  ...
  const { data: suggestions, isLoading: s_loading } = useQuery(
    "suggestions",
    getSuggestions,
    {
      enabled: !!user,
  ...
  return (
    <div
      id={increaseIndex ? "increaseIndex" : ""}
      style={{ width: "auto", paddingBottom: "3rem" }}
    >
      {/* <Section title={'Activity'} type = {'activity'} data = {activity} isLoading= {a_loading}/> */}
      <div className="beta-message-container">
        <span className="beta-message-title">
          <span
            class="iconify"
            data-icon="file-icons:test-generic"
            data-width="20"
          ></span>{" "}
          Beta warning!
        </span>
        <span className="beta-message-sub">
          Chakra is still in its beta phase, things might break, handle us with
          care
        </span>
      </div>
      <div style={{ padding: 0 }} id="section-not-sticky">
        ...
        >
          ...
          <span style={{ fontSize: "13px", marginBottom: "1rem" }}>
            Your personal Chakra frontpage. Come here to check in with your
