...
const AdminEdit = ({show, toggle, community}) => {
    ...
    const [spaceGuidelines, setSpaceGuidelines] = useState(community?.communityGuidelines)
  ...
  return (
    ...
    <div  id='e-modal' className='animate__animated animate__fadeIn animate__faster'>
      <span onClick={() => toggle(false)} class="close">&times;</span>
        ...
        <div style={{width: '100%', height: '75%', marginLeft: 'auto', padding: '1rem', overflowY: 'scroll'}} className='profile-section'>
          <div className='profile-elements'>
            <img className='profile-section-img' src={community?.communityIcon}/>
            <div id='e-modal-meta' >
              <span className='profile-section-name'>{community?.communityName}</span>
          ...
          <div className='space-description'>
            ...
            <input value={spaceGuidelines} onChange = {(e) => setSpaceGuidelines(e.target.value)}/>
            <span>Briefly explain what are the DOs and DON'Ts of this space</span>  
          </div>
        </div>
        ...
        {previewSource && <> <img style = {{width :'60px', height: "40px", marginLeft:'1rem', objectFit:'cover', padding: '0.2rem', marginTop: '1rem'}} src={previewSource}/> <span></span></>}
        {formState && <span style={{padding: '1rem', color: `${!spaceName || !spaceDesc ||!spaceGuidelines ? 'red': 'green'}`, fontSize: '0.9rem'}}>{!spaceName || !spaceDesc ||!spaceGuidelines? <i style={{marginRight: '0.5rem'}} class="fa-solid fa-triangle-exclamation"></i> : <i style={{marginRight: '0.5rem'}} class="fa-solid fa-circle-check"></i>}{formState}</span>}
    </div>
  </>
  )
}
