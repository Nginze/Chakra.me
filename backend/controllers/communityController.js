...
const createCommunity = async (req, res) => {
  ...
  const newCommunity = new Community({
    ...
    communityBanner:
      "https://res.cloudinary.com/chakra-me/image/upload/v1655306195/default_wallpaper_bcuctp.webp",
    communityBannerBlurred:
      "https://res.cloudinary.com/chakra-me/image/upload/w_1700,h_530,c_fill/e_blur:1000000000000/v1655306195/default_wallpaper_bcuctp.webp",
...
const getCommunityById = async (req, res) => {
  const community = await Community.findOne({ _id: req.params.id }).populate([
    "admins",
    "members",
  ]);
    ...
    await Community.updateOne(
      { _id: req.params.id },
      {
        $set: {
          ...
          communityIcon: result.secure_url,
        },
      }
    );
  } else {
...
const validateMember = async (req, res) => {
  ...
  res
    .status(200)
    .json({
      isMember: members.includes(req.query.user),
      isAdmin: admins.includes(req.query.user),
    });
};
...
const changeBanner = async (req, res) => {
  if (req.body.imgbase64) {
    ...
    await Community.updateOne(
      ...
      {
        $set: {
          communityBanner: result.secure_url,
          communityBannerBlurred: result.blurred_url,
        },
      }
    );
  }
